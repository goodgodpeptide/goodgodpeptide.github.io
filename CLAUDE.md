...
## 🔴 서브에이전트 진단 규칙 (최우선)
**같은 버그/오류를 2번 이상 고치려 할 때 → 반드시 서브에이전트로 재진단**

- 1차 시도 실패 → 원인 재분석 후 2차 시도 가능
- 2차 시도도 실패 → 즉시 서브에이전트(Explore 타입) 소환
  - 내 기존 판단/시도 내용 없이 관련 파일만 넘겨서 깨끗하게 진단
  - 서브에이전트 결과 받은 뒤 그 방향으로 수정
- GG가 "서브에이전트로 봐줘" 라고 말해도 즉시 소환
- 이유: 대화가 길어질수록 메인 Claude는 앞선 판단에 앵커링돼 같은 실수를 반복하는 경향이 있음

## 🤖 Claude 모델 선택 규칙 (필수)
- **복잡한 설계 / 반복 버그 / 개편 플랜** → 서브에이전트 `model: "opus"` (Claude Opus 4.7)
- **단순 코드 수정 / 파일 읽기 / 주석** → 기본 Sonnet 4.6 직접 처리
- **로그 분석 / 검색 / 파일 탐색** → Haiku 서브에이전트 (토큰 절약)
- 메인 Claude가 **같은 버그 2번 이상 실패** → 즉시 Opus 서브에이전트로 재진단
- 서브에이전트 진단 후에는 반드시 그 방향으로 구현 (기존 판단 고집 금지)
